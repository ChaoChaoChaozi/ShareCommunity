					...
					<div class="box">
						<c:if test="${event.object_type eq dic.object_type_post }">
							<a href="<c:url value="/post/${event.object_id }" />">
								<img src="<c:url value="http://image.share.com/${event.content }?imageView2/1/w/300/h/240" />" alt="" />
							</a>
						</c:if>
						<c:if test="${event.object_type eq dic.object_type_album }">
							<a href="<c:url value="/album/${event.object_id }/photos" />">
								<img src="<c:url value="http://image.share.com/${event.title }?imageView2/1/w/300/h/240" />" alt="" />
							</a>
						</c:if>
 						<div class="meta">
							<a href="<c:url value="/user/${event.user_id }" />">
								<img class="ui avatar image" src="http://image.share.com/${event.user_avatar}?imageView2/1/w/48/h/48">
					...
					<div class="row">
						<div>
							<c:forEach items="${tags }" var="tag" begin="0" end="9">
								<div class="tagbox">
									<div>
										<img class="visible" src="<c:url value="http://image.share.com/${tag.cover }?imageView2/1/w/200/h/200" />" alt="" />
				...
				<div class="container">
					<div class="row">
						...
						<c:forEach items="${feeds }" var="feed">
							<div class="userbox">
								<div class="header">
									<img class="avatar" src="http://image.share.com/${feed.key.user_avatar }" alt="" />
									...
									<c:forEach items="${feed.value }" var="f">
										<c:if test="${f.object_type eq dic.object_type_post }">
										   <a class="box" href="<c:url value="/post/${f.object_id }" />" href="<c:url value="/post/${f.object_id }" />">
												<img src="http://image.share.com/${f.content }${album_thumbnail}" alt="" />
												<div class="cover">
													${f.title }
												</div>
											</a>
										...
										<c:if test="${f.object_type eq dic.object_type_album }">
											<a class="box" href="<c:url value="/album/${f.object_id }/photos" />" href="<c:url value="/album/${f.object_id }/photos" />">
												<img src="http://image.share.com/${f.title }${album_thumbnail}" alt="" />
												<div class="cover">
													${f.summary }
												</div>		
											</a>							
